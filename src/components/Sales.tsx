...
export const Sales: React.FC<SalesProps> = ({ userRole, userName, finishedGoods, sales, customers, onCheckout, onMarkAsPaid, onDeleteSale }) => {
  ...
  return (
    <div className="p-1 md:p-6 h-full overflow-y-auto">
      {/* Header Area */}
          ...
          <div className="bg-white rounded-xl md:rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh] md:max-h-[90vh]">
            ...
            <div className="p-3 md:p-6 overflow-y-auto space-y-3 md:space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-4">
                ...
                <div className="md:col-span-2"><label className="block text-[9px] md:text-xs font-bold text-slate-500 mb-0.5 md:mb-1">GPS Location (📍 ကို နှိပ်ပါ)</label><div className="relative"><input type="text" value={gpsLocation} onChange={e => setGpsLocation(e.target.value)} placeholder="Latitude, Longitude" className="w-full bg-slate-50 border border-slate-200 p-1.5 md:p-3 pr-8 rounded-md md:rounded-lg outline-none focus:border-emerald-500 text-[10px] md:text-sm" /><button type="button" onClick={fetchLocation} title="Get Current Location" className="absolute right-2 top-1/2 -translate-y-1/2 text-rose-500 font-bold text-sm md:text-lg hover:scale-125 transition-transform active:scale-95">📍</button></div></div>
              </div>
              ...
              <div key={sale.id} className="bg-white p-2 md:p-5 rounded-lg md:rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-1.5 md:gap-4 hover:shadow-md transition-shadow">
                ...
                <div className="text-right w-full md:w-auto border-t border-slate-100 md:border-0 pt-1.5 md:pt-0 mt-1 md:mt-0 flex justify-between md:flex-col items-center md:items-end">
                  ...
                  <div className="flex gap-1 md:gap-2 justify-end w-full md:w-auto">
                     ...
                     <button onClick={() => window.print()} className="px-1.5 py-1 md:px-3 md:py-1.5 bg-slate-100 text-slate-600 font-bold rounded-md md:rounded-lg text-[9px] md:text-xs hover:bg-slate-200">🖨️ Print</button>
